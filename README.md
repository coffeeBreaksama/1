...
`2017/6/24`<br>
修改邮件审核模式，增加邮件审核快捷键。
此版本为初稿测试版本，正在和hxf敲定具体快捷键。
鼠标点击邮件条目（邮件具体内容区域无效）以选中邮件，选中之后邮件变色代表此封邮件已选中。
在选中邮件的前提下，按以下快捷键审核(比如说按5选中"标题聚类"，再按2选中"正常"并自动调至下一条)：
以下数字均为小键盘上的数字键，快捷键仅对当前选中条目生效。
//0 忽略 按下后将跳转并选中至下一条目
//1 正常 按下后将跳转并选中至下一条目
//2  广告邮件 按下后将跳转并选中至下一条目
//3  订阅邮件 按下后将跳转并选中至下一条目
//4  垃圾邮件 按下后将跳转并选中至下一条目
//5,I   标题聚类/URL/概要标题/趋势分析/文本 
//6,O   不入库
//9   拒收 按下后将跳转并选中至下一条目
