...
whileDelay();
    	...
    	var getmapurl = "/base/get-text-map.json";
    	...
    	if((map.productId==null || map.productId=='undefined')){
    		logger.error("访问错误!");
    		return;
    	}
        var init = function() {
        	$.ipAccountInit(8);
    		$.postForm(getmapurl, null, function(result){
				productMap = result['data'].productMap;
				targetNameMap  = result['data'].targetNameMap[map.productId];
				//酷狗这个线上用户等级选所有
				if(map.productId == 5177 || map.productId == 58117){
					$("#level").val("-1");
				}

				$("#targetId").addOption(targetNameMap, false);
				$("#targetId").chosen({no_results_text: "未找到匹配项"});
				query();
    		});
            $('.j-date').datetime();
	...
	var addListDialog=(function(){
		...
		$(dialogId+" input[name='listLevel'").on("click",function(){
			if($(this).val()=="productLevel"){
                ...
                $(dialogId+" #listType").selectOptions("2");
			}
