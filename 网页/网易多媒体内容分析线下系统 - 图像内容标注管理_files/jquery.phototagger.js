...
(function( window, $ ){
    ...
    function PhotoTagger( container, settings ){
        ...
        // I am the dialog form
        // for choose labels
        this.textTemp = '';
        this.typeKey = this.settings.currentLabelKey;
        this.initDialog();
        ...
        this.isActiveContainer = true; // 设置为true的话默认显示tags
        ...
        this.container.mousedown(
            function( event ){
                // 鼠标右键
                if (event.which == 3  && self.settings.currentLabelKey == self.settings.textLabelKey) {
                    // @todo 最好做一下事件解除
                ...
                if (
                    self.isTagCreationEnabled && !target.is( "b.photo-tagger-message" )
                ){
    ...
    PhotoTagger.prototype = {
        ...
        initDialog: function() {
            ...
            this.dialog = $(
                '<div id="j-dialog" class="m-layer z-show"><table><tbody><tr><td>\
                    <article class="lywrap">\
                    <section class="lyct" id="j-pt-labels"></section>\
                    <footer class="lybt">\
                        <div class="lybtns">\
                            <button type="button" class="u-btn" id="j-pt-ok">确定</button>\
                            <button type="button" class="u-btn u-btn-c4" id="j-pt-cancel">取消</button>\
                        </div>\
                    </footer>\
                    </article></td></tr></tbody></table>\
                </div>');

            this.labelCont = this.dialog.find('#j-pt-labels'); // 标签
            this.labelOk = this.dialog.find('#j-pt-ok'); // 确定
            this.labelCancel = this.dialog.find('#j-pt-cancel'); // 取消

            this.labelHTML = '';
            $.each(this.settings.labels, function(key, lab) {
                var clazz = self.settings.currentLabelKey === +key ? 'j-sel' : '';
                this.labelHTML += '<span class=\"'+ clazz +'\" data-key=\"' + key + '\">' + lab + '</span>';
            }.bind(this));
            if (this.settings.currentLabelKey === self.settings.textLabelKey) {
                this.labelHTML += '<input id="j-label-input"/>';
            } else {
                this.labelHTML += '<input style="display: none" id="j-label-input"/>';
            }
            this.labelCont.html(this.labelHTML);
            this.dialog.hide().appendTo(document.body);
            this.dialog.on('click', 'span[data-key]', function() { // 绑定tag点击事件
                var that = $(this);
                var ipt = $("#j-label-input"); // 输入框
                var key = that.data('key'); // 标签的key
                self.typeKey = key;
                that.parent().children('span').removeClass('j-sel');
                that.addClass('j-sel');
                ipt.val(that.html());
                if (that.data('key') == self.settings.textLabelKey) { // 文本内容默认选中
                    ipt.val('').show();
                } else {
                    ipt.hide();
                }}
        ...
        hideDialog: function() { // 隐藏标图会话框
            this.dialog.find('span[data-key]').removeClass('j-sel');
            this.dialog.find('#j-label-input').hide().val('');
            this.dialog.hide();
        },

        showDialog: function(callback) { // 显示标图会话框
            this.textTemp = '';
            this.typeKey = this.settings.currentLabelKey;
            this.dialog.find('span[data-key=\"'+ this.settings.currentLabelKey +'\"]').addClass('j-sel');
            if (this.settings.textLabelKey === this.settings.currentLabelKey) {
                this.dialog.find('#j-label-input').show();
                setTimeout(function() {
                    this.dialog.find('#j-label-input').focus();
                }.bind(this), 100);
                this.settings.showDetectText({
                    pid: this.settings.getPhotoID( this.container ),
                    left: this.pendingTag.position().left,
                    top: this.pendingTag.position().top,
                    width: this.pendingTag.width(),
                    height: this.pendingTag.height()
                }, function(text) {
                    $("#j-label-input").val(text);
                });
            } else {
                this.dialog.find('#j-label-input').val(this.settings.labels[this.typeKey]);
            }

            var okHandle = function() {
                this.textTemp = $("#j-label-input").val();
                this.hideDialog();
                callback.call(this);
            }.bind(this);
            ...
            $('#j-pt-ok').off('click').on('click', okHandle);
            $('#j-pt-cancel').off('click').on('click', cancelHandle);
        ...
        // I add a tag to the
        addTag: function( id, x, y, width, height, message, key ){
            var self = this;
            var param = {};
            ...
            // console.log("this.settings.textLabelKey "+ this.settings.textLabelKey);
            // console.log("this.settings.picMarkType "+ this.settings.picMarkType);
            // console.log("this.settings.textMarkType "+ this.settings.textMarkType);
            // if ((key == this.settings.textLabelKey && this.settings.picMarkType != this.settings.textMarkType)
            //     || (key != this.settings.textLabelKey && this.settings.picMarkType == this.settings.textMarkType)) {
            //     alert("标记类型错误");
            //     return;
            // }
        ...
        createTag: function( left, top, width, height, message ){
            // Create the new tag.
            var tag = $( "<a class='" + this.getFullClassName( "tag" ) + "'>" + (message ? '<b class=\"photo-tagger-message\">' + message + '</b>' : '')+ "</a>" );
        ...
        deleteTag: function( tag ){
            ...
            this.settings.deleteTagViaAjax({
                pid: this.settings.getPhotoID( this.container ),
                id: tag.data( "id" )
            }, function() {
        ...
        teardownPendingTagCreation: function(){ // 绘制tag
            var self = this;
            ...
            var addTagFn = function() {
                ...
                var tag = this.addTag(
                    ...
                    this.rightMouseDown ? '#' : this.textTemp,
                    this.rightMouseDown ? this.settings.textLabelKey : this.typeKey
                );
    ...
    $.fn.photoTagger.defaultOptions = {
        ...
        // I flag whether or not existing tag deletion is
        ...
        // 标签保存重复重写
        saveTagViaAjax: function() {},

        // 标签删除方法重写
        deleteTagViaAjax: function() {},

        // 获取标签列表方法重写
        getTagsViaAjax: function() {},

        // 回显检测值到输入框作为默认值
        showDetectText: function() {},
